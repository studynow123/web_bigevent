$(function() {
    ...
    function renderPage(total) {
        ...
        laypage.render({
            elem: 'pageBox', // 分页容器的 Id，注意这里的id不用加 # 号
            ...
            // 触发 jump 回调的方式有两种：
            // 1. 点击页码的时候，会触发 jump 回调.  此时first为false
            // 2. 只要调用了 laypage.render() 方法，就会触发 jump 回调. 此时first为true
            jump: function (obj, first) { // obj包含了当前分页的所有参数 
                // 可以通过 first 的值，来判断是通过哪种方式，触发的 jump 回调
                // 如果first的值为 true，证明是方式2触发的，否则就是方式1触发的
                console.log(first);
        ...
        layer.confirm('确认删除?', { icon: 3, title: '提示' }, function (index) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    layer.msg('删除成功！')
                    // 当数据删除完成后，需要判断当前这一页中，是否还有剩余的数据(通过删除按钮的个数来判断)
